...
A command-palette selection with an active thread invokes immediately: the plugin appends a visible, plugin-attributed user message instructing the agent to make the canonical `skill` tool call and then follow the loaded skill with a normal reply — the selection is treated as the user's request, so no follow-up submission is needed and the agent never goes silent after loading. A selection made with no active thread (Amp's welcome screen) creates a visible thread via `amp.getBuiltinAgent('medium').createThread({ show: true })` and invokes there the same way. Only when thread creation fails does the selection fall back to a reserved any-thread queue key, consumed by the first message of a thread the plugin has not yet seen an agent turn from — typically the thread that first message creates. Threads that already had a turn in this plugin process never consume it; a thread resumed after a plugin reload counts as unseen, which is the accepted residual. The threadless entry is cleared only when consumed or explicitly cancelled. The next `agent.start` for a thread consumes and clears that thread's own queue before returning invocation context. An explicit token in that same message takes precedence over the queue so user text wins; the thread's stale queued selection is also cleared.
...
- If no thread is active, the selection opens a new thread and invokes there; when thread creation fails, it queues for the first message of the next new thread.
